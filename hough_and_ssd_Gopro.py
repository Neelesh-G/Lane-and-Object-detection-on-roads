...
def draw_lines(img, lines, color=[255, 0, 0], thickness=2):
    ...
    for line in lines:
        for x1,y1,x2,y2 in line:
            ...
            if (slope > 0.3 and slope < 0.9  and ((x1 and x2) > 320)):
              #right lane
            ...
            elif (slope < -0.3 and slope > -0.9  and ((x1 and x2) < 320)): 
                #left lane
    ...
    if(np.isnan(meanSloper) or np.isnan(meanBr)):
      pass
        ...
        if(math.isfinite(y1) and math.isfinite(x2)):
            cv2.line(img, (x1, int(y1)), (int(x2), y2), [255, 0, 0], 6)
...
def process_image(image):
    ...
    edgesImage = canny(blurredImage, 60, 140)
    ...
    lineMarkedImage = hough_lines(regionInterestImage, 1, np.pi/180, 30, 15, 100)
...
# Calling and creating output video file
...
while success:
    ...
    image = cv2.resize(gopro, (640, 480), interpolation = cv2.INTER_AREA)
